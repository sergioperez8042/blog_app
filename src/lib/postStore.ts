...
export class PostStore {
  ...
  static getMostPopularPost(): Post | undefined {
    if (globalPosts.length === 0) return undefined;
    return globalPosts.reduce((mostPopular, current) => 
      (current.likes || 0) > (mostPopular.likes || 0) ? current : mostPopular
    );
  }

  static getPostsOrderedByLikes(): Post[] {
    return [...globalPosts].sort((a, b) => (b.likes || 0) - (a.likes || 0));
  }

  static getLatestPosts(limit: number = 5): Post[] {
    return [...globalPosts]
