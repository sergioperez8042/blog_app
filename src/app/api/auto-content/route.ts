...
class AutoContentScheduler {
  ...
  async generateDailyContent() {
    try {
      ...
      const newPost: Post = {
        ...
        likes: Math.floor(Math.random() * 50) + 1 // Likes iniciales aleatorios entre 1 y 50
      };
  ...
  async getLastRunDate(): Promise<string | null> {
    try {
      // En producción, usar almacén global
      if (process.env.NODE_ENV === 'production') {
        const autoPost = await PostStore.getLastAutoGeneratedPost();
        return autoPost ? new Date(autoPost.createdAt).toISOString() : null;
      }
      ...
      return autoPost ? autoPost.createdAt.toString() : null;
    } catch (error) {
      return null;
    }
  }
