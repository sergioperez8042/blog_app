...
class AutoContentScheduler {
  ...
  private inMemoryPosts: Post[]; // Para producción

  constructor() {
    this.postsFile = path.join(process.cwd(), 'data', 'posts.json');
    this.inMemoryPosts = []; // Inicializar array en memoria
  ...
  async savePost(post: Post) {
    try {
      // En producción, usamos un almacén temporal en memoria
      // En desarrollo, podemos usar archivos locales
      const isProduction = process.env.NODE_ENV === 'production';
      
      if (isProduction) {
        // Guardar en memoria para producción
        this.inMemoryPosts.unshift(post);
        
        // Mantener solo los últimos 50 posts
        if (this.inMemoryPosts.length > 50) {
          this.inMemoryPosts = this.inMemoryPosts.slice(0, 50);
        }
        
        console.log('💾 Post guardado exitosamente en memoria (producción)');
        return;
      }
      ...
      // No lanzar error en producción para evitar fallos
  ...
  async getLastRunDate(): Promise<string | null> {
    try {
      // En producción, usar almacén en memoria
      if (process.env.NODE_ENV === 'production') {
        const autoPost = this.inMemoryPosts.find((post: Post) => post.isAutoGenerated);
